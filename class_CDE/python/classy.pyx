    ...
    def get_current_derived_parameters(self, names):
        ...
        for name in names:
            if name == 'h':
                ...
                value = self.ba.Omega0_ncdm_tot*pow(0.7,2)*93.14
            elif name == 'Neff':
                value = self.ba.Neff      
            elif name == 'tau_reio':
            ...
            # NEW	
            #elif name == 'R_shift':
               # value = self.ba.h*100*self.th.da_rec*(1.+self.th.z_rec)*(self.ba.Omega0_b + self.ba.Omega0_cdm+self.ba.Omega0_ncdm_tot + self.ba.Omega0_dcdm)**0.5/299792.458
            # NEW
            elif name == 'beta_1':
            ...
            # NEW
                ...
                value = self.sp.sigma12
            elif name == 'S12':
                value = self.sp.sigma12*pow((self.ba.Omega0_b + self.ba.Omega0_cdm)*pow(self.ba.h,2)/0.14,0.4)
            # NEW
            elif name == 'S8':
                value = self.sp.sigma8*pow((self.ba.Omega0_b + self.ba.Omega0_cdm)/0.3,0.5)
            # NEW
            #elif name == 'betaArticle':
            #    value = self.ba.beta*pow(2,0.5)
            # NEW
            elif name == 'omega_cdm':
