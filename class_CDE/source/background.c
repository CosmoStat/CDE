...
int background_functions(
  ...
  /* photons */
  pvecback[pba->index_bg_rho_g] = pba->Omega_g_tilde * pow(70000/_c_,2) / pow(a_rel,4);
  ...
  if (pba->has_ur == _TRUE_) {
    pvecback[pba->index_bg_rho_ur] = pba->Omega_ur_tilde * pow(70000/_c_,2) / pow(a_rel,4);
...
int background_init(
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->N_ncdm > 0) {
      ...
      Neff = pba->Omega_ur_tilde/7.*8./pow(4./11.,4./3.)/pba->Omega_g_tilde;
  ...
  if ((pba->background_verbose > 0) && (pba->has_ncdm == _TRUE_)) {
    for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++) {
      printf(" -> non-cold dark matter species with i=%d has m_i = %e eV (so m_i / omega_i =%e eV)\n",
             ...
             pba->m_ncdm_in_eV[n_ncdm]*pba->deg_ncdm[n_ncdm]/pba->Omega0_ncdm[n_ncdm]/0.7/0.7);
    }
  }
...
int background_ncdm_M_from_Omega(
  ...
  rho0 = pow(70000/_c_,2)*pba->Omega0_ncdm[n_ncdm]; /*Remember that rho is defined such that H^2=sum(rho_i) */
  M = 0.0;
...
int background_initial_conditions(
  ...
  */
  ...
  rho_rad = Omega_rad*pow(70000/_c_,2)/pow(a/pba->a_today,4);
  if (pba->has_ncdm == _TRUE_){
  ...
  if (pba->has_dcdm == _TRUE_){
    ...
    pvecback_integration[pba->index_bi_rho_dcdm] =
      pba->Omega_ini_dcdm*pow(70000/_c_,2)*pow(pba->a_today/a,3);
  ...
  if (pba->has_cdm == _TRUE_){
    pvecback_integration[pba->index_bi_rho_cdm] =
      pba->Omega_cdm_tilde*pow(70000/_c_,2)*pow(pba->a_today/a,3);
  }
  /* */
  ...
  if (pba->has_dr == _TRUE_){
    if (pba->has_dcdm == _TRUE_){
       ...
       */
      f = 1./3.*pow(a/pba->a_today,6)*pvecback_integration[pba->index_bi_rho_dcdm]*pba->Gamma_dcdm/pow(70000/_c_,3)/sqrt(Omega_rad);
      pvecback_integration[pba->index_bi_rho_dr] = f*pow(70000/_c_,2)/pow(a/pba->a_today,4);
    }
    else{
...
int background_derivs(
  ...
  dy[pba->index_bi_D_prime] = (-a*H+pow(6,0.5)*scf_beta*y[pba->index_bi_phi_prime_scf])*y[pba->index_bi_D_prime] + 1.5*a*a*rho_M*y[pba->index_bi_D]*(1+4*pow(scf_beta,2));
  /* */
  ...
  /* NEW */
    ...
    dy[pba->index_bi_rho_cdm] = -3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*y[pba->index_bi_rho_cdm]-
      pow(6,0.5)*scf_beta*y[pba->index_bi_rho_cdm]*y[pba->index_bi_phi_prime_scf];
  }
  /* */
  ...
  if (pba->has_scf == _TRUE_){
    ...
    dy[pba->index_bi_phi_prime_scf] = - y[pba->index_bi_a]*
      (2*pvecback[pba->index_bg_H]*y[pba->index_bi_phi_prime_scf]
       + y[pba->index_bi_a]*dV_scf(pba,y[pba->index_bi_phi_scf])) + scf_beta*pow(6,0.5)*pvecback[pba->index_bg_rho_cdm]*pow(y[pba->index_bi_a],2) ;
  }
  /* */
...
double beta(
  ...
  //split beta into 3 tomographic bins 0<z<100, 100<z<1000, z>1000

  return (pba->beta_3/sqrt(2)-pba->beta_2/sqrt(2))*tanh(0.03*(z-1000))/2+(pba->beta_2/sqrt(2)-pba->beta_1/sqrt(2))*tanh(0.03*(z-100))/2+(pba->beta_3/sqrt(2)+pba->beta_1/sqrt(2))/2;
...
double beta_prime(
  ...
  return  0.03*((pba->beta_3/sqrt(2)-pba->beta_2/sqrt(2))/(2*cosh(0.03*(1000-z))*cosh(0.03*(1000-z)))+(pba->beta_2/sqrt(2)-pba->beta_1/sqrt(2))/(2*cosh(0.03*(100-z))*cosh(0.03*(100-z))));
