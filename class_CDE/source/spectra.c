...
int spectra_indices(
  ...
  /* NEW */
  class_define_index(psp->index_tr_delta_tot_no_scf,ppt->has_density_transfers,index_tr,1);
  /* */
...
int spectra_pk(
  ...
  /* NEW */
  class_call(spectra_sigma(pba,ppm,psp,12.,0.,&(psp->sigma12)),
             psp->error_message,
             psp->error_message);  
  /* */
...
int spectra_matter_transfers(
  ...
  /* NEW */
  double delta_cdm,delta_b,delta_g,delta_ncdm,delta_ur,delta_scf;
  double rho_cdm,rho_b,rho_g,rho_ncdm,rho_ur,rho_scf;
  /* */
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      for (index_ic = 0; index_ic < psp->ic_size[index_md]; index_ic++) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          /*psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_delta_tot] = 
          (rho_cdm*delta_cdm+rho_g*delta_g+rho_b*delta_b+rho_ncdm*delta_ncdm+rho_ur*delta_ur+rho_scf*delta_scf)/(rho_cdm+rho_g+rho_b+rho_ncdm+rho_ur+rho_scf); */ 
          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_delta_tot_no_scf] = 
          (rho_cdm*delta_cdm+rho_g*delta_g+rho_b*delta_b+rho_ncdm*delta_ncdm+rho_ur*delta_ur)/(rho_cdm+rho_g+rho_b+rho_ncdm+rho_ur);
          /* */
...
int spectra_output_tk_titles(struct background *pba,
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      /* NEW */
      class_store_columntitle(titles,"d_tot_no_scf",_TRUE_);    
      /* */
    ...
    /* NEW */
    class_store_columntitle(titles,"-T_tot_no_scf/k2",_TRUE_);
    /* */ 
  }
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          /* NEW */
          class_store_double(dataptr,tk[psp->index_tr_delta_tot_no_scf],_TRUE_,storeidx);
          /* */
        ...
        /* NEW */
        class_store_double_or_default(dataptr,-tk[psp->index_tr_delta_tot_no_scf]/k2,_TRUE_,storeidx,0.0);
        /* */
      }
    }
  }
