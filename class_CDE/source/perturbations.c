...
int perturb_get_k_list(
  ...
  /* NEW */
  /*ppt->k_max_for_pk = ppt->k_max_for_pk*(pba->h/0.7);*/
  ppt->k_max_for_pk = 2;
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (ppt->has_source_h == _TRUE_)         
       /* NEW */
       /* Apparently, one term is missing here, which involves the integration of the CDM velocity divergence in conformal time. I have explicitly checked that even setting this variable
       _set_source_(ppt->index_tp_h) to zero it doesn't appear any difference in the value of sigma8, for instance. Neither in the TT, EE, etc. Cl's. Thus, we can safely forget this issue.
       In hi_CLASS these variable is not even defined. If I directly remove this part of the code the program works perfectly fine. Apparently it does not play any role in any computation. */
    ...
    if (ppt->has_source_delta_scf == _TRUE_) {
    	/* NEW */
      if (ppt->gauge == synchronous){
        delta_rho_scf = 1./a2_rel*ppw->pvecback[pba->index_bg_phi_prime_scf]*y[ppw->pv->index_pt_phi_prime_scf]
           + ppw->pvecback[pba->index_bg_dV_scf]*y[ppw->pv->index_pt_phi_scf];
      }
      /* */
      else{
    ...
    if (ppt->has_source_theta_scf == _TRUE_) {
      rho_plus_p_theta_scf = k*k/a2_rel*ppw->pvecback[pba->index_bg_phi_prime_scf]*y[ppw->pv->index_pt_phi_scf];
