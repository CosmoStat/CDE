...
int input_init(
  ...
  char * const target_namestrings[] = {"Omega_dcdmdr","omega_dcdmdr",
                                         "Omega_ini_dcdm","omega_ini_dcdm","sigma8"};
  char * const unknown_namestrings[] = {"Omega_ini_dcdm","Omega_ini_dcdm",
                                           "Omega_dcdmdr","omega_dcdmdr","A_s"};
  enum computation_stage target_cs[] = {cs_background, cs_background, cs_background,
                                         cs_background, cs_spectra};
...
int input_read_parameters(
  ...
  class_test(class_at_least_two_of_three(flag1,flag2,flag3),
             errmsg,
             "In input file, you can only enter one of T_cmb, Omega_g_tilde or omega_g_tilde, choose one");
  ...
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_b_tilde or omega_b_tilde, choose one");
  if (flag1 == _TRUE_)
  ...
  if (class_none_of_three(flag1,flag2,flag3)) {
    pba->Omega_ur_tilde = 3.046*7./8.*pow(4./11.,4./3.)*pba->Omega_g_tilde;
  }
  else {
  ...
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_cdm_tilde or omega_cdm_tilde, choose one");
  if (flag1 == _TRUE_)
 ...
 /**/
  ...
  pba->K = -pba->Omega0_k*pow(pba->a_today*(70000/_c_),2);
  /** - Set curvature sign */
...
int input_default_precision ( struct precision * ppr ) {
  ...
  /*second_order_CLASS;first_order_CAMB*/
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case Omega_dcdmdr:
      ...
      if (ba.has_dr == _TRUE_)
        ...
        rho_dr_today = 0.;
      output[i] = (rho_dcdm_today+rho_dr_today)/(pow(70000/_c_,2))-pfzw->target_value[i];
      break;
    case omega_dcdmdr:
      rho_dcdm_today = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_dcdm];
      if (ba.has_dr == _TRUE_)
        ...
        rho_dr_today = 0.;
      output[i] = (rho_dcdm_today+rho_dr_today)/(pow(70000/_c_,2))-pfzw->target_value[i]/pow(0.7,2);
      break;
    case Omega_ini_dcdm:
    case omega_ini_dcdm:
      rho_dcdm_today = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_dcdm];
      if (ba.has_dr == _TRUE_)
        ...
        rho_dr_today = 0.;
      output[i] = -(rho_dcdm_today+rho_dr_today)/(pow(70000/_c_,2))+ba.Omega0_dcdmdr;
      break;
    case sigma8:
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_dcdmdr:
      ...
      */
      gamma = ba.Gamma_dcdm/pow(70000/_c_,1);
      if (gamma < 1)
        a_decay = 1.0;
      else
